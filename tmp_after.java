for (var e : windows) {
                java.time.LocalTime s = e.getKey().getKey();
                java.time.LocalTime tEnd = e.getKey().getValue();
                // skill windows first (deterministic order by skillId nulls last -> globals後回しにするとスキル不足時にグローバルで埋められる)
                java.util.List<java.util.Map.Entry<Key,Integer>> perSkill = new java.util.ArrayList<>(e.getValue().entrySet());
                ...
                for (var e2 : perSkill) {
                    ...
                    results.addAll(assignSeatsForSlot(day, s, tEnd, skill, seats, pool,
